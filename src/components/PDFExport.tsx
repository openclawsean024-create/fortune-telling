...
// Pre-create styles once (outside component to avoid recreation)
...
export default function PDFExport({ report }: PDFExportProps) {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      // Use pdf() function instead of PDFDownloadLink to avoid lazy-render hang
      const blob = await pdf(<FortunePDF report={report} />).toBlob();

      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `命理報告-${report.id}.pdf`;
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`inline-flex items-center gap-2 py-2 px-4 ${bgColor} text-white font-medium rounded-lg transition-all`}
    >
      {label}
    </button>
  );
